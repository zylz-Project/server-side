...
三个产品固件会把响应中的 `revision` 保存到 ESP32 NVS（命名空间 `audio_sync`）。
当服务端清单版本变化时，ESP32 会刷新现有文件，而不只比较“文件名 + 文件大小”。
所以管理员用同名、同大小的新音频覆盖旧文件后，设备也能取得新内容。只有全部下载和
Flash TOC 写入成功后，ESP32 才保存新版本；如果清单 JSON 损坏、字段不完整或存在重复文件名，
ESP32 会终止本次同步，不会删除设备上已有的音频。
